...
class Miner:
    def __init__(self):
        ...
        if os.path.exists(os.path.join(path, 'Blockchain_System', 'Miner_0', 'blockchain.json')) == False:
            ...
            f_prev = os.path.join(path, miners[-1], 'blockchain.json')
            f_curr = os.path.join(path, f"Miner_{int(miners[-1].split('_')[1])+1}", 'blockchain.json')

            shutil.copy(f_prev, f_curr)
        
        self.path = os.path.join(str(Path(__file__).parent).split('Miner_0')[0], 'Miner_0') # Change here for other Miners
